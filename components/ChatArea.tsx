    ...
    return (
      <div 
        className={`flex-1 flex flex-col items-center justify-center p-8 relative ${
          isNotion ? 'bg-notion-bg dark:bg-notion-darkbg' : 'bg-slate-50 dark:bg-forsion-darkbg'
        }`}
        ...
        {isDragging && (
           <div className="absolute inset-0 bg-forsion-500/10 dark:bg-forsion-500/20 backdrop-blur-sm z-50 flex items-center justify-center border-4 border-dashed border-forsion-400 rounded-xl m-4">
             <div className="text-center animate-bounce">
                ...
                <h3 className="text-2xl font-bold text-forsion-700 dark:text-forsion-200">Drop image here</h3>
             </div>
           </div>
        )}

        <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-8 animate-fade-in-up ${
           isNotion 
              ...
              className={`px-5 py-4 transition-all duration-200 ${
                isNotion
                  ...
                  : `rounded-2xl shadow-sm ${msg.role === 'user' 
                      ? 'bg-forsion-600 text-white rounded-tr-sm shadow-forsion-500/20' 
                      : msg.isError
                        ? 'bg-red-50 border border-red-200 text-red-800 dark:bg-red-900/20 dark:border-red-800 dark:text-red-200'
                        : 'bg-white text-slate-800 border border-gray-100 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-700 rounded-tl-sm shadow-md shadow-gray-200/50 dark:shadow-none'}`
              }`}
            >
              {/* User Attachments */}
